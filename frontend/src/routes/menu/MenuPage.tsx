	...
	return (
		<div className='w-full'>
			<div className='p-4 flex w-full gap-10'>
				...
				<Button className='bg-[hsla(126,100%,24%,0.69)] rounded-full w-32'>Tìm Kiếm</Button>
			</div>
				...
				<div className='bg-black h-1/4 rounded-lg w-full'>
					<div className='text-white flex flex-row  h-full p-8 gap-4 justify-between relative'>
						...
						<div className='relative'>
							<div className='bg-[hsla(126,100%,24%,0.69)] absolute left-[-30px] rounded-2xl py-2 px-4'>Giảm 30%</div>
							<img
								className='flex'
					...
					<div className='flex flex-col p-2 gap-4  rounded-lg border-2 '>
						...
						<div className='flex flex-col text-lg'>
							<div className='flex justify-between'>
								...
								<span>{products.map((item) => item.count * item.price).reduce((prev, curr) => prev + curr, 0)}</span>
							</div>
							...
							<div className='flex justify-between text-lg'>
								...
								<span>{products.map((item) => item.count * item.price).reduce((prev, curr) => prev + curr, 0)} vnd</span>
							</div>
						</div>
						<div className='border'> </div>
						...
						<div>
							<Button className=' text-xl border-2 rounded-3xl bg-[#5ec7e4] text-black hover:bg-[#5ec7e4] w-60 h-16  flex justify-between'>Thanh toán</Button>
						</div>
						<div className='border '> </div>
					</div>
				</div>
...
function MenuItems({ type }: MenuItemProps) {
	const { data, isLoading, isError } = useQuery('products', () => getProducts(type));
