...
export default function Admin() {
	...
	return (
		<nav className='flex items-start justify-center md:justify-start w-full h-full'>
			<div className='font-[500] bg-blue-500/90 p-2 gap-2 text-xl flex flex-col justify-start items-start h-full min-w-[200px]'>
				{paths.map((item, index) => (
					<Link
						key={index}
						className='hover:text-[hsla(29,90%,58%,1)]'
						to={item.path}
					>
